...
const HomePage = () => {
  const features = [
    { icon: <Truck className="text-walmart-blue" size={28} />, title: 'Free Shipping', desc: 'On orders above ₹500' },
  ...
  return (
    <div className="animate-fade-in bg-slate-50 min-h-screen pb-20">
      {/* Announcement Bar */}
      <div className="bg-walmart-blue text-white text-[10px] font-black uppercase tracking-[0.2em] py-2 text-center overflow-hidden">
        <div className="flex justify-center gap-12 whitespace-nowrap animate-marquee">
          <span>⚡ Flash Sale Live: Up to 70% Off</span>
          <span>🚚 Free Shipping on orders over ₹500</span>
          <span>💎 Premium Quality Guaranteed</span>
          <span className="hidden md:inline">⚡ Flash Sale Live: Up to 70% Off</span>
        </div>
      </div>
          ...
          >
            ...
            <div className="max-w-7xl mx-auto px-4 h-full flex items-center relative z-10">
              <div className="max-w-2xl space-y-8">
                 <div className={`inline-block px-4 py-1.5 rounded-xl font-black text-[10px] uppercase tracking-widest bg-white/20 backdrop-blur-md text-white border border-white/20 animate-slide-up`}>
                    {slide.badge}
                 </div>
                 <div className="space-y-4">
                    <h1 className="text-5xl md:text-8xl font-black leading-[0.85] tracking-tighter text-white animate-slide-up" style={{ animationDelay: '100ms' }}>
                    ...
                    <p className="text-xl text-white/80 font-medium max-w-sm animate-slide-up" style={{ animationDelay: '200ms' }}>
                      {slide.subtitle}
                    </p>
                 </div>
                 <div className="flex flex-wrap gap-4 pt-4 animate-slide-up" style={{ animationDelay: '300ms' }}>
                    <Link 
                      to={slide.link} 
                      className="bg-walmart-yellow text-slate-900 px-10 py-4 rounded-full font-black text-sm uppercase tracking-widest hover:brightness-110 transition-all shadow-xl shadow-black/20 transform hover:-translate-y-1"
                    >
                       {slide.btnText}
                    </Link>
                    <Link 
                      to="/products" 
                      className="bg-white/20 backdrop-blur-md text-white px-10 py-4 rounded-full font-black text-sm uppercase tracking-widest hover:bg-white/30 transition-all"
                    >
                       Explore Deals
                    </Link>
                 </div>
      ...
      <main className="max-w-7xl mx-auto px-4 py-10 space-y-16">
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-8">
           <div className="rounded-[2.5rem] overflow-hidden relative group h-[400px]">
              <img src="https://images.unsplash.com/photo-1542291026-7eec264c27ff?q=80&w=2000&auto=format&fit=crop" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" alt="Nike" />
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent flex flex-col justify-end p-10 text-white">
                 <h2 className="text-4xl font-black tracking-tight mb-2">RUN YOUR WAY.</h2>
                 <p className="text-slate-200 mb-6 font-medium">New arrivals from Nike are here.</p>
                 <Link to="/products?brand=nike" className="bg-white text-slate-900 w-fit px-8 py-3 rounded-full font-bold text-sm tracking-wide hover:bg-slate-100 transition-all">Shop Nike</Link>
              </div>
           </div>
           
           <div className="rounded-[2.5rem] overflow-hidden relative group h-[400px]">
              <img src="https://images.unsplash.com/photo-1511385348-a52b4a160dc2?q=80&w=2000&auto=format&fit=crop" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" alt="Apple" />
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent flex flex-col justify-end p-10 text-white">
                 <h2 className="text-4xl font-black tracking-tight mb-2">M3 CHIP IS HERE.</h2>
                 <p className="text-slate-200 mb-6 font-medium">Experience the future of MacBook.</p>
                 <Link to="/products?category=electronics" className="bg-white text-slate-900 w-fit px-8 py-3 rounded-full font-bold text-sm tracking-wide hover:bg-slate-100 transition-all">Learn More</Link>
              </div>
           </div>
        </section>
             ...
             {allProducts.slice(0, 8).map((product) => (
                <Link key={product._id} to={`/products/${product.slug}`} className="shrink-0 w-48 bg-white border border-slate-100 p-4 rounded-3xl hover:shadow-xl transition-all">
                   <div className="aspect-square bg-slate-50 rounded-2xl overflow-hidden mb-3">
                      <img src={product.images?.[0]?.url || 'https://via.placeholder.com/400'} className="w-full h-full object-cover group-hover:scale-105 transition-transform" alt={product.name} />
                   </div>
